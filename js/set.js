...
function setBgImgInit() {
  ...
  switch (bg_img['type']) {
    ...
    case '3':
      ...
      break
    case '4':
      $('#bg').attr('src', 'https://api.toshiki.dev') //随机二次元
      break
    case '5':
      $('#bg').attr('src', 'https://r18.toshiki.dev') // random r18 anime
      break
    case '6':
...
$(document).ready(function () {
  ...
  $(document).on('click', function (e) {
    // 选择搜索引擎点击
    if (
      ($('.search-engine').is(':hidden') && $('.se').is(e.target)) ||
      ($('.search-engine').is(':hidden') && $('#icon-se').is(e.target))
    ) {
  ...
  $('.se_list_table').on('click', '.set_se_default', function () {
    ...
    iziToast.show({
      timeout: 8000,
      message: 'Would you like to set as default engine?',
      buttons: [
        [
  ...
  $('.se_add_save').click(function () {
    ...
    if (!num.test(key)) {
      iziToast.show({
        timeout: 2000,
        message: 'Order ' + key + ' is not positive integer',
      })
      return
    }
  ...
  $('.se_list').on('click', '.delete_se', function () {
    ...
    if (key == se_default) {
      iziToast.show({
        message: 'Default engine cannot be removed ',
      })
    } else {
      iziToast.show({
        timeout: 8000,
        message: 'Confirm deletion of' + 'engine' + key,
        buttons: [
          [
            '<button>Confirm</button>',
            function (instance, toast) {
  ...
  $('.set_se_list_preinstall').click(function () {
    iziToast.show({
      timeout: 8000,
      message: 'Data of current search engine will be deleted',
      buttons: [
        [
          '<button>Confirm</button>',
          function (instance, toast) {
  ...
  $('.quick_add_save').click(function () {
    ...
    if (quick_list[key]) {
      iziToast.show({
        timeout: 8000,
        message: 'Short " + key + " has existing data, confirm overwriting？',
        buttons: [
          [
            '<button>Confirm</button>',
            function (instance, toast) {
              ...
              iziToast.show({
                message: 'Successfully overwrited',
              })
            },
            true,
          ],
  ...
  $('.set_quick_list_preinstall').click(function () {
    iziToast.show({
      timeout: 8000,
      message: 'Shortcut data will be reomved',
      buttons: [
        [
          '<button>Confirm</button>',
          function (instance, toast) {
  ...
  $('.quick_list').on('click', '.delete_quick', function () {
    ...
    iziToast.show({
      timeout: 8000,
      message: 'Confirm deletion of' + ' shortcut' + key,
      buttons: [
        [
          '<button>Confirm</button>',
          function (instance, toast) {
  ...
  $('#wallpaper').on('click', '.set-wallpaper', function () {
    ...
    if (type === '1') {
      $('#wallpaper_text').html('Display default wallpaper, refresh to take effect')
      setBgImg(bg_img)
      iziToast.show({
        message: 'Successfully set wallpaper, refresh to take effect',
      })
    }

    if (type === '2') {
      $('#wallpaper_text').html('Display Bing daily wallpaper, refresh to take effect')
      setBgImg(bg_img)
      iziToast.show({
        message: 'Successfully set wallpaper, refresh to take effect',
      })
    ...
    if (type === '3') {
      $('#wallpaper_text').html('Display random scenery photo, refresh to take effect') // Display random scenery photo, refresh to take effect
      setBgImg(bg_img)
      iziToast.show({
        message: 'Successfully set wallpaper, refresh to take effect',
      })
    ...
    if (type === '4') {
      $('#wallpaper_text').html('Display random anime photo, refresh to take effect')
      setBgImg(bg_img)
      iziToast.show({
        message: 'Successfully set wallpaper, refresh to take effect',
      })
    ...
    if (type === '5') {
      $('#wallpaper_text').html('Display random R18 anime photo, refresh to take effect')
      setBgImg(bg_img)
      iziToast.show({
        message: 'Successfully set wallpaper, refresh to take effect', // Successfully set wallpaper, refresh to take effect
      })
    }

    if (type === '6') {
      $('#wallpaper_text').html('Custom wallpaper URL, please enter valid URL, click save and refresh to take effect') // Custom wallpaper URL, please enter valid URL, click save and refresh to take effect
      $('#wallpaper_url').fadeIn(100)
      $('#wallpaper-button').fadeIn(100)
      $('#wallpaper-url').val(bg_img['path'])
    } else {
  ...
  $('.wallpaper_save').click(function () {
    ...
    if (!reg.test(url)) {
      ...
      setBgImg(bg_img)
      iziToast.show({
        message: 'Custom wallpaper successfully set, refresh to take effect', // Custom wallpaper successfully set, refresh to take effect
      })
    }
  })
  ...
  $('#my_data_out').click(function () {
    ...
    download('俊樹のStart-config-backup-' + $.now() + '.json', json)
    iziToast.show({
      timeout: 2000,
      message: 'Already exported backup config to download folder', // Already exported backup config to download folder
    })
  })
  ...
  $('#my_data_file').change(function () {
    ...
    reader.onload = function () {
      ...
      if (typeof mydata != 'object') {
        iziToast.show({
          timeout: 2000,
          message: 'Config data format error',
        })
        return
      }
      ...
      iziToast.show({
        timeout: 8000,
        message: 'Existing data will be overwritten, confirm import?', // existing data will be overwritten, confirm import?
        buttons: [
          [
            '<button>Confirm</button>',
            function (instance, toast) {
